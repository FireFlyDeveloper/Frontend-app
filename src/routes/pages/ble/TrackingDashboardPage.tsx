      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="rounded-lg border bg-card p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Transporting</p>
              </div>
            </div>
            <div className="rounded-lg border bg-card p-4 flex items-center gap-4">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList>
              <TabsTrigger value="overview"><MapPin className="h-4 w-4 mr-2" />Rooms</TabsTrigger>
              <TabsTrigger value="items"><Package className="h-4 w-4 mr-2" />Items</TabsTrigger>
              <TabsTrigger value="devices"><Radio className="h-4 w-4 mr-2" />Devices</TabsTrigger>
                      ...
                      <Card key={room.id} className="group relative">
                        <CardContent className="pt-4 pb-4">
                          ...
                          <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); openRoomEdit(room) }}>
                              <Pencil className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={(e) => { e.stopPropagation(); handleRoomDelete(room.id) }}>
                    ...
                    <Card key={tag.id}>
                      <CardContent className="flex items-center justify-between py-3">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          {tag.item_id ? (
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleUnassign(tag)} title="Unassign"><Unlink className="h-4 w-4 text-muted-foreground" /></Button>
                          ) : (
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openAssign(tag)} title="Assign"><Link2 className="h-4 w-4 text-muted-foreground" /></Button>
                          )}
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openTagEdit(tag)}><Pencil className="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => handleTagDelete(tag.id)}><Trash2 className="h-4 w-4" /></Button>
                        </div>
                      </CardContent>
                    </Card>
                  ))}
